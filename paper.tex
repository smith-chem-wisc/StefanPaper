...
\begin{figure}
  \includegraphics[scale=0.7]{figureDifferentSearchModes.eps}
...
A major drawback of this approach is that it misses monoisotope errors and co-isolated peptides.
To avoid those complications, and to get a more comprehensive list of matches, a wider search tolerance is used, e.g. $\pm 3.5$ Da.
This gives a substantially larger list of matches, and all the mass shifts could be attributed to co-isolation or missed monoisotopes.
A post-processing step may be employed to confidently identify the co-isolated and missed monoisotope matches, but a wide range of high-confidence matches with un-identified mass shifts remain.
These often correspond to modifications on the identified peptides, and require an extensive framework (such as G-PTM-D) to identify and label them.
...
We observe that when using an incomplete database that does not include comprehensive modification information, a notch search with notches at monoisotopic peaks is superior to either of the narrow-mass searches.
...
PSMs Within 1\% FDR & 157188      & 178003        & 173483       \\
MisAssigned PSMs    & 0           & 7783          & 0            \\
Good PSMs           & 157188      & 170220        & 173483       \\ 
\end{tabular}
\end{table}
