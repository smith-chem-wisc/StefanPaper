...
Some people use extra software for identifying good search parameters: the calibration procedure in MetaMorpheus outputs those file-specific tolerances as a side product of the calibration method.
...
One example of multi-notch search is the \textit{limited multi-notch} search, with notches at masses corresponding to Dalton differences between the monoisotopic peak and other peaks in averagine isotopic envelopes (e.g. 1.0029, 2.0054).
...
With G-PTM-D added modifications, the correct peptide is identified with sequence \texttt{[Ammonia los]QAFDDAIAELDTLNEDSY[Oxidation]K}. It has a mass difference of -0.00171(-0.79458 ppm) and a score 19.222.
While this kind of misassignment is not possible in the 5ppm narrow-window search, often missed monoistopes are assigned to a wrong sequence.
...
These simple errors are relatively easy to spot, but not when there are missed cleavages with multiple amino acids and/or mulitple PTMs.
...
\begin{table}[]
\centering
\caption{Number of PSMs in Multi-Notch Search vs. Open Search for select mass differences. These PSMs are shown as mass difference histograms in Figure~\ref{fig:fig6-ZeroAndPhospho}}
...
\caption{Histogram of PSM counts for select mass differences around zero, sulfo, and phospho masses. The Multi-Notch searches produce visibly more PSMs.}
...
\caption{m/z peaks in MS1 scan 12333}
...
However, all peptides in the range[725.4, 727.9] are co-fragmented and a better solution would be to use all co-isolated precursor masses.
...
Six, the small list of 13 PTMs does not include many important modifications.
...
These enhancements demonstrate that the enhanced G-PTM-D approach can and should be used whenever bottom-up proteomics is done.
It is fast enough to be part of every regular work-flow.
And, the significant contribution of correctly identified PTM peptides is clearly beneficial to biologists seeking to understand their data better.
...
\end{document}
ven though adducts arising from experimental artifacts are not ultimately interesting to biologists, not including them as an option decreases the total number of identified proteins and PTMs.
The overall identification rate decreases from 220,574 to 217,597 peptide-spectral matches (PSMs), which in turn decreased the number of identified modified peptides by 4\%, see Table~\ref{tab:table3};
